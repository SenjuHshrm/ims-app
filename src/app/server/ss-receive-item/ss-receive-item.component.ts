...
@Component({
  selector: 'app-ss-receive-item',
  templateUrl: './ss-receive-item.component.html',
  styleUrls: ['./ss-receive-item.component.scss']
})
export class SsReceiveItemComponent implements OnInit {
  ...
  add(obj: any) {
    if(+obj.remainingItem < 0) {
      ...
      this.searchItem.addRec(data).subscribe(res => {
        if(res.res) {
